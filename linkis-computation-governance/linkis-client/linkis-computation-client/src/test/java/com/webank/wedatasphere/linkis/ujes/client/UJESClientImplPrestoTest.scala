...
object UJESClientImplPrestoTest extends App {

  val gatewayUrl = "http://10.63.118.84:9001"
  val user = "europa"
  val password = "europa"

  val clientConfig = DWSClientConfigBuilder.newBuilder().addServerUrl(gatewayUrl)
    ...
    .setAuthenticationStrategy(new StaticAuthenticationStrategy()).setAuthTokenKey(user)
    .setAuthTokenValue(password).setDWSVersion("v1").build()
  val client = UJESClient(clientConfig)

  val jobExecuteResult = client.execute(JobExecuteAction.builder().setCreator("UJESClient")
    .addExecuteCode("show catalogs")
    .setEngineType(EngineType.PRESTO)
    .setRunType(EngineType.PRESTO.PSQL)
    .setUser(user).build())
